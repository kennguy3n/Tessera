...
#[napi(object)]
/// JS-facing view of an [`Automation`]: identity, JSON-encoded
/// trigger/action, and run-state metadata.
pub struct AutomationInfo {
    /// Automation id, stringified.
    pub id: String,
    ...
    /// Whether the runner will dispatch this automation.
    pub enabled: bool,
    /// When the automation was created, RFC 3339.
    pub created_at: String,
    /// When the automation was last updated, RFC 3339.
    pub updated_at: String,
    /// When the automation last ran (RFC 3339), or `None` if never.
    pub last_run_at: Option<String>,
    /// Outcome of the last run (e.g. `"ok"` / `"error"`), if any.
...
pub struct CreateAutomationRequest {
    /// Human-readable automation name.
    pub name: String,
    /// JSON that must parse as an `AutomationTrigger`.
    pub trigger_json: String,
    /// JSON that must parse as an `AutomationAction`.
    pub action_json: String,
...
/// Creates an automation from a renderer request and returns it.
...
/// Returns every registered automation.
