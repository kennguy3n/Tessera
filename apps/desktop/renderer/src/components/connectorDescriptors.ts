...
export const CONNECTOR_DESCRIPTORS: ConnectorDescriptor[] = [
  ...
  },
  {
    ...
    label: "Discord",
    category: "Chat",
    keywords: ["discord", "channel", "messages", "guild", "bot"],
    consoleUrl: "https://discord.com/developers/applications",
    help: "Create a Discord application, add a Bot, and reset its token. Invite the bot to your server with only the read-only View Channels + Read Message History permissions. Paste the bot token and the Channel ID to index.",
    secretRequired: false,
    reads: [
      "Messages and their authors in the configured Discord channel (read-only)",
      "Message timestamps and metadata",
    ],
    neverTouches: [
      "Sending, editing, or deleting messages",
      "Any channel other than the one you configure",
      "Server settings, roles, or member management",
      ...READ_ONLY_GUARANTEES,
    ],
  },
  {
    ...
    label: "Bitbucket",
    category: "Code",
    keywords: ["bitbucket", "atlassian", "git", "repos", "pull requests", "code"],
    consoleUrl: "https://support.atlassian.com/bitbucket-cloud/docs/create-a-repository-access-token/",
    help: "Create a Repository (or Workspace) Access Token with the read-only repository + pullrequest scopes. Paste the token along with the Workspace ID and Repository slug to index.",
    secretRequired: false,
    reads: [
      "Pull request titles, descriptions, and discussion in the configured repository (read-only)",
      "Repository metadata",
    ],
    neverTouches: [
      "Creating, editing, merging, or declining pull requests",
      "Pushing code or changing repository settings",
      "Any repository other than the one you configure",
      ...READ_ONLY_GUARANTEES,
    ],
  },
  {
    ...
    label: "Airtable",
    category: "Storage",
    keywords: ["airtable", "base", "table", "records", "database", "spreadsheet"],
    consoleUrl: "https://airtable.com/create/tokens",
    help: "Create a personal access token with the read-only data.records:read + schema.bases:read scopes, granted to your base. Paste the token along with the Base ID and table name to index.",
    secretRequired: false,
    reads: [
      "Record values in the configured base and table (read-only)",
      "The table's field names and base schema",
    ],
    neverTouches: [
      "Creating, editing, or deleting records",
      "Any base or table other than the one you configure",
      ...READ_ONLY_GUARANTEES,
    ],
  },
  {
    ...
    label: "Monday.com",
    category: "Issues",
    keywords: ["monday", "boards", "items", "work os", "project management"],
    consoleUrl: "https://developer.monday.com/apps/docs/oauth",
    help: "Create a monday.com OAuth app, add the redirect URI below, and grant the read-only boards:read + me:read scopes, then copy the Client ID and Client Secret. Enter the Board ID to index.",
    secretRequired: true,
    reads: [
      "Item names, column values, and updates on the configured board (read-only)",
      "Board and group metadata",
    ],
    neverTouches: [
      "Creating, editing, or deleting items or columns",
      "Any board other than the one you configure",
      ...READ_ONLY_GUARANTEES,
    ],
  },
