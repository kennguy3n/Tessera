...
describe("useSkillRunner", () => {
  ...
  it("runs all steps sequentially and threads outputs into context", async () => {
    ...
    const { result } = renderHook(() => useSkillRunner(DOCUMENT_DELIBERATE_DRAFT));

    act(() => {
      result.current.run({ topic: "Our refund policy", context: "30 day window." });
    });
    ...
    expect(result.current.contextVars.draft_text).toBe("Draft body about the topic.");
