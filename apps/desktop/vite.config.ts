...
export default defineConfig(({ mode }) => {
  // Build-time guard: the showcase bridge (seeded demo data) must only ever
  // enter the dedicated `--mode qa` bundle. The `main.tsx` guard relies on
  // Vite statically replacing `VITE_TESSERA_QA` so the branch tree-shakes out
  // of `npm run build`, but that safety holds only as long as no env file
  // sets the flag for a non-QA mode. Rather than trust that by convention,
  // fail the build loudly here if the flag is ever set outside `mode === "qa"`
  // (e.g. a stray `VITE_TESSERA_QA=true` in `.env`/`.env.production`), which
  // would otherwise silently ship the showcase code into the shipped renderer.
  const env = loadEnv(mode, path.resolve(__dirname, "renderer"), "VITE_");
  if (mode !== "qa" && env.VITE_TESSERA_QA === "true") {
    throw new Error(
      `VITE_TESSERA_QA=true is only valid for the showcase build (\`--mode qa\`), ` +
        `but it is set while building mode="${mode}". This would bundle the ` +
        `showcase mock bridge and seeded demo data into a production renderer. ` +
        `Remove it from the .env file for this mode — only renderer/.env.qa may set it.`,
    );
  }
