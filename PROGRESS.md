...
| 10 | Export failure recovery + retry queue — failed exports persist to a dedicated `failed-exports.json` envelope under `{userData}` (split from `config.json` for lifecycle independence — a corrupt config never wipes the user's pending retries, and queue churn never bloats the renderer's config reads) with `{ id, artifactId, format, filePath, errorMessage, failedAt, retryCount }`; `artifacts:failedExports` / `artifacts:retryExport(id)` / `artifacts:discardFailedExport(id)` IPC drive the Settings "Failed exports" card. Vitest covers queue persistence, FIFO eviction at `FAILED_EXPORTS_MAX = 100`, retry-count bumping, dequeue-on-success, structural-validation of tampered on-disk entries, write-side absolute-path validation, and concurrent-write serialisation. | `DONE` (PR 2) |
